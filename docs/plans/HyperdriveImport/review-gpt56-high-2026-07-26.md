...
Evidence — `applyBuild` begins with clean creation defaults, stores only purchased characteristic/skill deltas in the actor source, and lets species, free ranks, and learned-tree effects supply the rest ([apply-build.js:84](../../../modules/char-creator/apply-build.js#L84), [apply-build.js:96](../../../modules/char-creator/apply-build.js#L96), [apply-build.js:139](../../../modules/char-creator/apply-build.js#L139)). Matched compendium items retain their Active Effects through `toItemData` and `projectItemSource` ([to-item-data.js:84](../../../modules/char-creator/to-item-data.js#L84), [build-item-schema.js:104](../../../modules/char-creator/build-item-schema.js#L104)). Species effects include starting characteristics and thresholds; Brawn modifiers also expand into soak and encumbrance ([import-helpers.js:3071](../../../modules/importer/import-helpers.js#L3071), [modifiers.js:502](../../../modules/helpers/modifiers.js#L502)).
...
The claimed automatic recomputation is also false. `prepareDerivedData` does not reconstruct wounds, strain, soak, or defence from final characteristics; `_calculateDerivedValues` only recomputes current encumbrance ([actor-ffg.js:183](../../../modules/actors/actor-ffg.js#L183), [actor-ffg.js:612](../../../modules/actors/actor-ffg.js#L612)). Purchased Brawn’s wound/soak/encumbrance source deltas are explicitly added by `applyCharacteristicDeltas` ([actor-ffg.js:929](../../../modules/actors/actor-ffg.js#L929)).
...
Evidence — `materializeTreePurchases` compares `learnedKeys` directly against keys such as `talent0` or `upgrade3`, then overwrites every node’s `islearned` state ([item-helpers.js:441](../../../modules/helpers/item-helpers.js#L441), [item-helpers.js:456](../../../modules/helpers/item-helpers.js#L456)). OggDude specialization import creates `talent${row*4+column}` node keys independently from the talent’s OggDude content key ([specializations.js:80](../../../modules/importer/oggdude/importers/specializations.js#L80), [specializations.js:123](../../../modules/importer/oggdude/importers/specializations.js#L123)). Force-power and signature-ability nodes similarly become `upgradeN` ([forcepowers.js:146](../../../modules/importer/oggdude/importers/forcepowers.js#L146), [signature-abilities.js:133](../../../modules/importer/oggdude/importers/signature-abilities.js#L133)).
...
Evidence — Dedication is represented by a characteristic-specific attribute injected into the exact purchased node before materialization ([apply-build.js:31](../../../modules/char-creator/apply-build.js#L31), [to-item-data.js:51](../../../modules/char-creator/to-item-data.js#L51)). `learnedKeys` alone cannot express that characteristic choice.

Ranked talents are counted by learning each concrete occurrence. Actor preparation aggregates same-named ranked nodes across specializations ([actor-ffg.js:394](../../../modules/actors/actor-ffg.js#L394), [actor-ffg.js:410](../../../modules/actors/actor-ffg.js#L410)). A content key appearing twice in one tree or across multiple trees cannot identify which occurrences are learned. Force-power and signature-ability trees both use `upgradeN`, so node keys are meaningful only with their owning item.

The current `applyBuild` also handles Dedication choices only on the selected starting specialization; extra specializations receive `learnedKeys` but no `nodeAttributeGrants` ([apply-build.js:145](../../../modules/char-creator/apply-build.js#L145), [apply-build.js:156](../../../modules/char-creator/apply-build.js#L156)).
...
Evidence — `projectItemSource` merely whitelists and deep-clones `name/type/img/system/effects/flags`; it performs no validation, normalization, or effect generation ([build-item-schema.js:94](../../../modules/char-creator/build-item-schema.js#L94)). The OggDude import pipeline creates a temporary document and then explicitly runs effect synchronization after importing it ([import-helpers.js:2324](../../../modules/importer/import-helpers.js#L2324), [import-helpers.js:2340](../../../modules/importer/import-helpers.js#L2340)).

Embedded items created as part of an Actor normally skip `_onCreateAEs` because `options.parent` is true ([item-ffg.js:93](../../../modules/items/item-ffg.js#L93)). Consequently, a fallback species/career/specialization containing attributes but no prebuilt effects will not grant characteristics, skills, career flags, or modifiers. A tree stub with no grid also has nowhere to represent learned nodes.

Fix — Do not call the current OggDude `Import` methods directly; they are XML/zip/UI/compendium-writing orchestration ([weapons.js:16](../../../modules/importer/oggdude/importers/weapons.js#L16)). Extract shared pure translators and effect-source builders from those importers, then use them for both OggDude and Hyperdrive. Validate the final item through a temporary Item/Actor construction and a live prepared-data test. Unmatched tree items should be a user-confirmed partial import unless enough data exists to build the actual grid.
...
Fix — Extract a lower-level shared `assembleCharacterSource` used by both `applyBuild` and Hyperdrive. The wizard adapter supplies purchase-history-derived base values; Hyperdrive supplies normalized persisted values and accounting overrides. Reuse unsaved-Actor preview and item identity/projection helpers. Generalize commit normalization if used: its current XP log and `pcWizardCommit` stamp explicitly describe character creation, not an external import ([commit-normalize.js:71](../../../modules/char-creator/commit-normalize.js#L71)).

2. **XP must be source-authoritative, but the proposed `EarnedXP → total` mapping is not justified.**

Evidence — The actor model stores only `experience.total` and `available`; it derives neither ([character.js:52](../../../modules/data/models/actor/character.js#L52)). Wizard total means species starting XP plus bonus/extra grants, while available subtracts priced purchases ([calculators.js:22](../../../modules/char-creator/calculators.js#L22)). The design inventory lists `XP` and `EarnedXP` but does not identify an actual “used XP” key or establish whether `EarnedXP` includes species starting XP.
...
Evidence — `SOURCE_DESCRIPTORS` contains no talent or signature-ability pool; it explicitly calls those settings “not consumed” ([source-descriptors.js:12](../../../modules/char-creator/source-descriptors.js#L12), [source-descriptors.js:33](../../../modules/char-creator/source-descriptors.js#L33)). Calling `loadSource` for such a pool throws. `loadSource` also applies rarity/restricted gates inappropriate for resolving gear already owned by an imported character ([load-source.js:58](../../../modules/char-creator/load-source.js#L58), [load-source.js:66](../../../modules/char-creator/load-source.js#L66)).

The claimed broad lookup ignores locked packs in its cached path ([import-helpers.js:243](../../../modules/importer/import-helpers.js#L243), [import-helpers.js:246](../../../modules/importer/import-helpers.js#L246)).
...
- Force rating belongs at persisted `stats.forcePool.max`, while effective rating also includes active item effects ([actor-templates.js:140](../../../modules/data/actor-templates.js#L140), [actor-ffg.js:751](../../../modules/actors/actor-ffg.js#L751)). Copying final Force Rating into the base and embedding Force-rating effects double-counts it.
- Current encumbrance is recomputed from carried item quantity/equipment state, but only when the relevant calculation setting invokes `_calculateDerivedValues` ([actor-ffg.js:271](../../../modules/actors/actor-ffg.js#L271), [actor-ffg.js:617](../../../modules/actors/actor-ffg.js#L617)).
...
- `system.general.notes` is explicitly undeclared and will not persist under the DataModel ([actor-templates.js:229](../../../modules/data/actor-templates.js#L229), [actor-templates.js:240](../../../modules/data/actor-templates.js#L240)).
...
Evidence — Native items grant career-skill flags through inherent effects ([item-helpers.js:103](../../../modules/helpers/item-helpers.js#L103), [item-helpers.js:125](../../../modules/helpers/item-helpers.js#L125)). Wizard free ranks are separate item effects, while only XP-purchased ranks are stored directly on the actor ([to-item-data.js:27](../../../modules/char-creator/to-item-data.js#L27), [apply-build.js:104](../../../modules/char-creator/apply-build.js#L104)).
...
Evidence — `Attachments`, `UsedQualities`, and `WeaponModifiers` are not all `itemattachment[]`. Qualities/modifiers live in `system.itemmodifier[]`; attachments live in `system.itemattachment[]` ([item-templates.js:120](../../../modules/data/item-templates.js#L120), [item-templates.js:135](../../../modules/data/item-templates.js#L135)). Attachment application also needs a concrete target item; native wizard purchases use an explicit `attachTo` identifier ([apply-build.js:167](../../../modules/char-creator/apply-build.js#L167)). A flat weapon key is ambiguous when the actor owns two copies with different modifications.

Nested attachments require stable `_id` values for later editing ([item-editor.js:303](../../../modules/items/item-editor.js#L303)); the wizard identity helper explicitly assigns them ([build-item-schema.js:337](../../../modules/char-creator/build-item-schema.js#L337)).
...
Evidence — It calls the materializer only when `options.materializeTree` is a function ([to-item-data.js:91](../../../modules/char-creator/to-item-data.js#L91)). The wizard gets that binding from `makeBuildDependencies` ([build-deps.js:43](../../../modules/char-creator/build-deps.js#L43)), but section 8’s proposed call omits it.
...
The repository confirms only the Foundry side of `ffgimportid`: OggDude base objects store `obj.Key` there ([import-helpers.js:2281](../../../modules/importer/import-helpers.js#L2281)). It cannot confirm:
