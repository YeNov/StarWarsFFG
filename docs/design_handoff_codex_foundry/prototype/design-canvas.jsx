...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useLayoutEffect(() => {
    ...
    // Safety net: a restored (or stale) transform can place ALL content
    // outside the viewport, leaving the user staring at a blank canvas with
    // no obvious way back. After layout settles, if the world's rendered
    // bounds don't intersect the viewport at all, snap back to the default
    // origin view so something is always visible.
    const recenterIfLost = () => {
      const vp = vpRef.current, world = worldRef.current;
      if (!vp || !world) return;
      const v = vp.getBoundingClientRect(), w = world.getBoundingClientRect();
      if (w.width < 2 || w.height < 2) return; // not laid out yet
      const offscreen = w.right < v.left + 8 || w.left > v.right - 8 ||
        w.bottom < v.top + 8 || w.top > v.bottom - 8;
      if (offscreen) { tf.current = { x: 0, y: 0, scale: 1 }; apply(); }
    };
    const rafId = requestAnimationFrame(() => requestAnimationFrame(recenterIfLost));
    // Flush on pagehide and unmount so a reload within the 200ms debounce
    // window doesn't drop the last pan/zoom.
    window.addEventListener('pagehide', flush);
    return () => { cancelAnimationFrame(rafId); window.removeEventListener('pagehide', flush); flush(); };
  }, []);
