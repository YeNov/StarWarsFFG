...
* Changed — **Enable Ammo no longer disappears from a weapon's Sheet Options when the world uses the Limited Ammo quality**. It is shown greyed out with a note that ammo comes from the quality, so it is clear why it can't be ticked; the weapon's own setting is kept for if the world setting is turned off again.
...
* Fixed — **large quantities overflowed their Codex inventory card**, running over the − and + buttons, into ENC and across the item's name. The QTY column now fits up to four digits; a longer count shows its first three digits and ".." (e.g. 100..), with the full number on hover.
