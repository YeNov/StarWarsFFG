...
* Changed — **the Codex vehicle sheet's Silhouette, Speed, Handling and Armour row now spans the full width** above the stats and the Defence panel, instead of sitting in the left column. A minion vehicle group's Codex sheet also leaves out the System Strain box, as a minion sheet has no strain box, so Group Strength takes that row.
