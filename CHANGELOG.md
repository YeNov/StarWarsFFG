...
* Changed — **the Group Manager's Obligation, Duty and Morality tables now sit in tabs** instead of one long column, one tab for each table the group uses, and the tab you pick stays picked while the window refreshes. Group Actions moved up, above the Player Characters table.
* Fixed — **the Group Manager's Obligation and Duty tables filled up with copies of the same entries**, gaining another full set each time the window refreshed, which it does whenever any character changes. Worse, a character who had left the group kept their entries, so the d100 could still trigger an Obligation or Duty for someone no longer playing. Each table now lists every entry once, for the current group only.
