...
* Fixed — **two hits landing at the same moment could apply only one lot of damage**. Two players clicking Apply Damage on the same target within a moment of each other — 5 wounds and 7 — left it on 7 instead of 12, whether the GM applied it or a player forwarded it. Damage, crits and minion kills are now applied to a target one after another.
  * Forwarded requests are also checked before the GM acts on them: they must come from a connected player acting on their own attack card, they may only touch a wound or strain pool, and the GM whisper is now addressed and attributed by the GM rather than by whatever the sending client asked for.
* Fixed — **Destiny points could go missing when two people acted at once**. Two players rolling for the pool at the start of a session, or a flip landing while the GM was adding a point, each ended up counted once instead of twice, because every request read the pool and then wrote back a total it had worked out before the other one landed. Requests are now handled strictly one at a time, and a flip is checked against the pool as it stands at that moment — so flipping the last point twice is refused instead of driving the pool below zero, and whoever asked is told why.
  * The "Remaining" totals on a flip's chat card are now read back after the flip is applied rather than worked out in advance by the player who asked, so they match the tracker even when several people flip at once.
