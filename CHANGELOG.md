...
* Added — **Failure and Threat status effects**, for the next check or for the rest of the combat, alongside the existing Boost, Setback, Success and Advantage ones in the token HUD. Each adds one Failure or Threat to every check the character makes; the next-check version is removed once they roll, and the combat version once they leave the combat or it ends.
* Changed — **the token HUD's dice statuses are grouped** so they're easier to find: first the dice added to the next check, then the dice added for this combat, then the fixed results (Success, Advantage, Failure, Threat) for the next check and for this combat.
* Changed — **the Success and Advantage "This Combat" icons now leave a thin gap around their arrow**, as the new Failure and Threat ones do, so the arrow stays distinct from the symbol on the dark token HUD.
...
* Added — **small craft now defend in two zones, fore and aft** ([#82](https://github.com/YeNov/StarWarsFFG/pull/82)), following the rules for silhouette 4 and below — on both vehicle sheets and on the roll dialog's zone card. Every vehicle also gets a Zones picker on its sheet: Auto reads it off the silhouette, or force Two or Four for an oddball craft. The ratings of zones that are out of play are kept rather than cleared, and come back if you set the craft to four zones again.
