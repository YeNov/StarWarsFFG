...
* Fixed — **Obligation and Duty entries now count towards a character's Obligation and Duty.** The number on the sheet used to be a separate figure that no entry ever changed. It is now a baseline: outside Edit Mode each box shows the baseline plus every entry's magnitude, read-only, and in Edit Mode you edit the baseline. Characters built with the wizard or an importer keep the number they had, since their entries come in at 0. Morality and Conflict are unchanged and still set by hand.
